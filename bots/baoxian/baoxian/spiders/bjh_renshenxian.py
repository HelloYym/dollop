...
from baoxian.items import RenshenxianItem
import json


class BjhRsxSpider(scrapy.Spider):
    name = "bjh_renshenxian"
                ...
                for tr in tbody.xpath('tr'):
                    if len(tr.xpath('td')) == 3:
                        ...
                        try:
                            if get_content(tr.xpath('string(td[1])').extract()).find(u'资') >= 0:
                                ...
                                name = get_content(tr.xpath('string(td[2])').extract())
                                income = get_content(tr.xpath('string(td[3])').extract())
                                baohu_xz = get_content(tr.xpath('string(td[4])').extract())
                                duli_xz = get_content(tr.xpath('string(td[5])').extract())
                        except:
                            pass

                    elif len(tr.xpath('td')) == 6:
                        try:
                            if get_content(tr.xpath('string(td[1])').extract()).find(u'资') >= 0:
                                capital_structure = get_content(tr.xpath('string(td[1])').extract())
                        except:
                        ...
                        name = get_content(tr.xpath('string(td[3])').extract())
                        income = get_content(tr.xpath('string(td[4])').extract())
                        baohu_xz = get_content(tr.xpath('string(td[5])').extract())
                        duli_xz = get_content(tr.xpath('string(td[6])').extract())
                    ...
                    if name and income and name.find(u'公司名称') < 0 and name.find(u'小计') < 0 and name.find(u'合计') < 0:
                        ...
                        if flag:
                            company['share'] = share
                        if baohu_xz != None:
                            company['baohu_xz'] = baohu_xz
                        if duli_xz != None:
                            company['duli_xz'] = duli_xz
                        yield company
